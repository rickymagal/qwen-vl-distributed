...
int main(int argc, char** argv) {
  ...
  std::fprintf(stderr,
               "[stage0] device=%lld stages=%lld idx=%lld layers=[%d,%d)\n",
  ...
  auto input_ids = torch::randint(0, cfg.vocab_size, {B, T}, opts_i64);
  ...
  if (out.hidden_out.defined()) {
    ...
    std::fprintf(stderr, "[stage0] hidden_out: %s\n", s.c_str());
  }
  if (out.logits.defined()) {
    std::string s = c10::str(out.logits.sizes());
    std::fprintf(stderr, "[stage0] logits: %s\n", s.c_str());
  }
